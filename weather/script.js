...
async function updateFiveDays({ lat, lon }) {
  ...
  const fiveDays = await data.list.filter((item, index) => {
    ...
    let currDt = new Date(data.list[index]['dt_txt']).getDay();
    let prevDt = new Date(data.list[index - 1]['dt_txt']).getDay() || -1;

    if (currDt != prevDt) return item;
  });
  const fiveDaysHtml = [];
